$(document).on('click','.btn-qv', function() {
	var QVrecalc = 0;
	if (!$('#modalQV').length) {
		$('#content').append('<div class="modal fade" id="modalQV" tabindex="-1" role="dialog" aria-labelledby="'+$(this).attr('id')+'"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><div class="h3 modal-title">'+$(this).text()+'</div><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button></div><div class="modal-body quick-view"></div></div></div></div>');
		QVrecalc = 1;
	}
	//
