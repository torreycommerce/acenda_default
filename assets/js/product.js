	...
	*/
	this.updateChips = function(inorganic){
		...
		if(filteredVariants.length == 1){
			...
			//
			if(filteredVariants[0].has_stock != true || this.getNumber(filteredVariants[0].price) <= 0) {
				this.disabled = true;
...
var prodExtras = function () {
	...
	$(document).on('click','.stock .btn-eis', function() {
		...
		if (!email || !validateEmailAddress(email)) {
			...
			$('body').append('<div class="flash-note alert alert-danger"><div class="modal-header"><div class="h3 modal-title">Stock Notification Error</div><a href="#" class="close" data-dismiss="alert" aria-label="close stock notification error alert"><span aria-hidden="true">&times;</span></a></div><div class="modal-body">Please enter a valid email address.</div></div>');
			return false;
		}
	...
	function submitInStockEmail(email, variant_id)
	{
		$.post(acendaBaseUrl + '/api/instockemail', {
			...
			$('body').append('<div class="flash-note alert alert-danger"><div class="modal-header"><div class="h3 modal-title">Stock Notification Error</div><a href="#" class="close" data-dismiss="alert" aria-label="close stock notification error alert"><span aria-hidden="true">&times;</span></a></div><div class="modal-body">' + error + '</div></div>');
		});
	}
