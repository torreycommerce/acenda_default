...
(function ($) {
	...
	checkout.CheckoutView = Backbone.View.extend({
		...
		stripe_cardnumber: null,
		stripe_cardexpiry: null,
		stripe_cardcvc: null,				
		...
		findStartStep: function() {
			...
			if(that.getUrlParam('resourcePath')) {
				that.start_step = 'review';
				console.log('placing order again..');
			}			
			...
			_.each(this.checkout_steps,function(step) {
				if(!stop) {
					...
					if(step.name == that.start_step) {						
						if(that.getUrlParam('resourcePath')) {
							if(that.logged_in) {
				                $('input[name=email]').val(that.customer.get('email'));
							} 							
							$('.checkoutapp #checkout-steps').hide();
							$('.checkoutapp #summary-panel').hide();
							$('.checkoutapp #processing').show();
		...
		render: function () {
		    ...
		    if(this.shipping_countries !==null && $('#shipping-country').children().length<2 ) {
				...
				if(that.shipping_countries.length) {
					currentCountry = that.shipping_countries.first().get('value');
				}
		    ...
		    if(this.billing_countries !==null && $('#billing-country').children().length<2 ) {
				...
				if(that.billing_countries.length) {
					currentCountry = that.billing_countries.first().get('value');
				}				
				if(!currentCountry) currentCountry='US';		    	
		...
		setupStripe: function() {
			...
			$.post(acendaBaseUrl+'/api/stripe/getpk', function(data) {
					...
					that.stripe_elements = that.stripe.elements();
						...
						that.stripe_cardnumber = that.stripe_elements.create('cardNumber',{style: that.stripe_style,showIcon: true});
						that.stripe_cardnumber.mount('#card-number');
						that.stripe_cardexpiry =that.stripe_elements.create('cardExpiry',{style: that.stripe_style});
						that.stripe_cardexpiry.mount('#card-expiry');
						that.stripe_cardcvc = that.stripe_elements.create('cardCvc',{style: that.stripe_style});
						that.stripe_cardcvc.mount('#card-cvc');
					// Add an instance of the card Element into the `card-element` <div>.

	
					that.stripe_cardnumber.addEventListener('change', function(event) {
					  var displayError = document.getElementById('card-errors');

					  that.keyUpCardNumber(event.brand);
					  if (event.error) {
						$(displayError).removeClass('d-none').text(event.error.message)
					  } else {
						 $(displayError).addClass('d-none').text('')
					  }
					});	
					that.stripe_cardnumber.addEventListener('focus', function(event) {
  			...
  			$.post(acendaBaseUrl+'/api/braintree/token', function(data) {
		        ...
		        braintree.dropin.create({
				  ...
				  dataCollector: {
					...
					that.bt_dropin_instance.on('paymentMethodRequestable', function (event) {
					  console.log(event.type); 
					  $('#payment-validation-error').html('');
			...
			$.post(acendaBaseUrl + '/api/address/verify',formData).done(function(response) {
				...
				if(formData[formData.step + '_street_line1'].toUpperCase().trim() == addy['street_line1'] &&
				    ...
				    formData[formData.step + '_zip'] == addy['zip'].trim()) {
									console.log('tie',form_elem.find('.address-verify'));
                    form_elem.find('.address-verify').html('<input name="verified" value="1" type="hidden"/>');
	    ...
	    keyUpCardNumber: function(cardType) {

	    	if(typeof cardType == 'undefined' || typeof cardType == 'object') {
		...
		fetchShippingMethods: function(callback) {
            ...
            that.shipping_methods.reset();
            that.shipping_methods.set(that.cart.get('shipping_methods'));
            console.log('shipping_methods',that.shipping_methods);
	        ...
	        that.shipping_methods.each(function(method,k) {
        	    ...
        	    that.shipping_methods.at(k).set('price',method.get('shipping_rate'));
        	    console.log('set rate',method.get('shipping_rate') );
                $('#shipping-methods').html(tpl({methods: that.shipping_methods.toJSON(), current_method: that.cart.get('shipping_method')}));
                $('#shipping-methods .d-none').each(function() {
                	$(this).parents('span').find('.val').text($(this).text());
                });
		...
		buildCheckoutForm: function() {
			...
			checkoutForm = $.extend(checkoutForm,{payment_return_url: returnUrl });	
			if(!checkoutForm.billing_country) {
				if(that.billing_countries.length) {
					checkoutForm.billing_country = that.billing_countries.first().get('code');
				}
			}
			
			if(that.getUrlParam('resourcePath')) {
			     checkoutForm = $.extend(checkoutForm,{payment_method_nonce: that.getUrlParam('resourcePath')});
			}		
		...
		changedShippingMethod: function() {
			...
			if(method) {
		     	...
		     	this.cart.set('discount_price',method.get('discount_price'));
		     	this.cart.set('subtotal',method.get('subtotal'));	     		     	
		     	this.cart.set('total',method.get('total'));
		     	this.cart.set('tax_rate',method.get('tax_rate'));   
		     }  	
			this.summaryView.render();
			this.renderShippingMethodSummary();
		},
		changedSavedAddress: function(e,current) {
			...
			var copy_fields = ['first_name', 'last_name' ,'street_line1','street_line2','city','state','zip','phone_number','country'];
		...
		renderShippingMethodSummary:  function() {
			...
			if(typeof method !=='undefined') {
				...
				var temp = $('#shipping-methods .val:first').parent('span').html();
		...
		checkPayment: function(e) {
			...
			{
				if($('input#usenonce').prop("checked") || acendaIsLoggedIn == false) {
					  that.stripe.createToken(that.stripe_cardnumber).then(function(result) {
					    if (result.error) {
					      // Inform the customer that there was an error.
					      var errorElement = document.getElementById('card-errors');
						  $(errorElement).removeClass('d-none').text(result.error.message)
					    } else {
			    ...
			    that.bt_dropin_instance.requestPaymentMethod(function(err, payload) {			    	  
			    	    if(typeof payload == 'undefined' ) {
			    	    	$('#payment-validation-error').html('Please choose a valid payment method');
			    	    	return;
			    	    }
		...
		placeOrder: function(e) {
			...
			if(!that.getUrlParam('resourcePath')) {
    			if($('#ageverify').length) {
                    $('#ageverify').click(function() {
    		         	$('#review-panel #error-text').html('');
                    });
    				if(!$('input#ageverify').is(':checked')) {
    					$('#review-panel #error-text').html('<p>Please verify your age.</p>');
    					return;
    				}
    			}
    			if($('#customverify').length) {
                    $('#customverify').click(function() {
    		         	$('#review-panel #error-text').html('');
                    });
    				if(!$('input#customverify').is(':checked')) {
    					$('#review-panel #error-text').html('<p>'+ acendaCustomVerifyError +'</p>');
    					return;
    				}
    			}
			}
			...
			$.post(acendaBaseUrl + '/api/order/place',form).done(function(response) {
				...
				if(typeof response.doRedirect !=='undefined' && response.doRedirect == true) {
					$('body').prepend('<form id="redirectform" method="POST" action="'+ response.result.url +'"></form>');
					 $.each(response.result.parameters, function(n,v) {
					 		$('#redirectform').append('<input type="hidden" name="' + v.name +'" value="' + v.value + '"/>');			
				     });
					 console.log("submitting", $('#redirectform').html());
					 $('#redirectform').submit();
				} else {
						...
						if(typeof response.error == 'string') {
							...
							for (var property in response.error) {
							    if (response.error.hasOwnProperty(property)) {
						        	elem += '<li>'+property.replace('_',' ').toLowerCase().replace(/^(.)|\s(.)/g, function($1) { return $1.toUpperCase(); } ) + ' - '+response.error[property]+'</li>';
							    }
							}
							elem += '</ul>';
