...
function updateCartTotals(qtyField, cartItemId) {
	$.getJSON(acendaBaseUrl + '/api/sessioncart')
		...
		.done(function (data) {
			...
			if (data.result.shipping_methods) {
				$.each(data.result.shipping_methods, function (key, method) {
					if (method.bottom_days_range && method.top_days_range) {
						$('tr[data-method="' + method.id + '"] .wsnw').html(method.bottom_days_range + " - " + method.top_days_range + " days");
					}
					$('tr[data-method="' + method.id + '"] .shrink').html("$" + method.shipping_rate);
				});
			}
		});
}
